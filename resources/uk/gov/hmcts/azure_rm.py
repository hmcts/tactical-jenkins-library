...
AZURE_RETURN_HOST_VAR environment variable:
Host var to return, default azure_rm.py provides 'name'

This also sorts dict keys before output,
and also tries to sort hosts inside each dict
"""
AZURE_RETURN_HOST_VAR=os.environ.get("AZURE_RETURN_HOST_VAR", "name")
...
class AzureInventory(object):
    ...
    def _load_machines(self, machines):
        for machine in machines:
            ...
            host_vars = dict(
                ...
                computer_name=(machine.os_profile.computer_name if machine.plan else None),
            ...
            # Add windows details
            if 'windows_configuration' in dir(machine.os_profile) and machine.os_profile.windows_configuration is not None:
    ...
    def _add_host(self, vars):
        ...
        if self.group_by_resource_group:
            ...
            self._inventory[resource_group].append(vars[AZURE_RETURN_HOST_VAR])
            self._inventory[resource_group] = sorted(self._inventory[resource_group])

        if self.group_by_location:
            if not self._inventory.get(vars['location']):
                self._inventory[vars['location']] = []
            self._inventory[vars['location']].append(vars[AZURE_RETURN_HOST_VAR])
            self._inventory[vars['location']] = sorted(self._inventory[vars['location']])
        ...
        if self.group_by_security_group and security_group:
            ...
            self._inventory[security_group].append(vars[AZURE_RETURN_HOST_VAR])

        self._inventory['_meta']['hostvars'][host_name] = vars
        self._inventory['azure'].append(vars[AZURE_RETURN_HOST_VAR])
        self._inventory['azure'] = sorted(self._inventory['azure'])
        ...
        if self.group_by_tag and vars.get('tags'):
            for key, value in vars['tags'].items():
                ...
                self._inventory[safe_key].append(vars[AZURE_RETURN_HOST_VAR])
                self._inventory[safe_key] = sorted(self._inventory[safe_key])
                self._inventory[safe_value].append(vars[AZURE_RETURN_HOST_VAR])
                self._inventory[safe_value] = sorted(self._inventory[safe_value])
    ...
    def _json_format_dict(self, pretty=False):
        ...
        if pretty:
            ...
            return json.dumps(self._inventory, sort_keys=True)
